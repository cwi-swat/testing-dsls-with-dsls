...
def register_myql_oracle(driver):
    ...
    assert state['price'] == 100 if True else True, ' price was not the same as 100, expected ' + 100 + ', got ' + state['price']
    ...
    assert state['discount'] == 5 if (True and (state['acm'] and state['student'])) else True, ' discount was not the same as 5, expected ' + 5 + ', got ' + state['discount']
    ...
    assert state['discount'] == 10 if ((True and not (((state['acm'] and state['student'])))) and state['acm']) else True, ' discount was not the same as 10, expected ' + 10 + ', got ' + state['discount']
    ...
    assert state['discount'] == 20 if (((True and not (((state['acm'] and state['student'])))) and not ((state['acm']))) and state['student']) else True, ' discount was not the same as 20, expected ' + 20 + ', got ' + state['discount']
    ...
    assert state['discount'] == 0 if (((True and not (((state['acm'] and state['student'])))) and not ((state['acm']))) and not ((state['student']))) else True, ' discount was not the same as 0, expected ' + 0 + ', got ' + state['discount']
    ...
    assert state['pay'] == (state['days'] * ((state['price'] - state['discount']))) if True else True, ' pay was not the same as days * (price - discount), expected ' + (state['days'] * ((state['price'] - state['discount']))) + ', got ' + state['pay']
